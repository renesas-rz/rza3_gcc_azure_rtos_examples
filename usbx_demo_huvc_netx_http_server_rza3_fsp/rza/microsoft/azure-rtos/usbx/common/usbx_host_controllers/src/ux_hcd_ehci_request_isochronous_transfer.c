...
UCHAR                           start = UX_FALSE;
    ...
    if (endpoint -> ux_endpoint_device -> ux_device_speed != UX_HIGH_SPEED_DEVICE)
        head = &((UX_EHCI_FSISO_TD *)lp.sitd_ptr) -> ux_ehci_fsiso_td_transfer_head;
    else
#endif
    {

        /* Get pointer of ED.  */
        ied = ((UX_EHCI_HSISO_TD *)lp.itd_ptr) -> ux_ehci_hsiso_td_ed;
    ...
    if (request_list == UX_NULL)
    {
        ...
        /* Link to head.  */
        ((*(ULONG*)head)) = (ULONG)transfer_request;
        ((*(ULONG*)tail)) = (ULONG)transfer_request;
        ((*(ULONG*)first_new)) = (ULONG)transfer_request;
    }
    else
    {

        /* Link to tail of the list.  */
        ((UX_TRANSFER*)(*(ULONG*)tail)) -> ux_transfer_request_next_transfer_request = transfer_request;

        /* In case there is nothing to load, set new ones.  */
        if ((*(ULONG*)first_new) == UX_NULL)
            (*(ULONG*)first_new) = (ULONG)transfer_request;
    }

    /* Move tail until it's real last one.  */
#if 1
    while(((UX_TRANSFER*)(*(ULONG*)tail)) -> ux_transfer_request_next_transfer_request != UX_NULL)
        ((*(ULONG*)tail)) = (ULONG)(((UX_TRANSFER*)(*(ULONG*)tail)) -> ux_transfer_request_next_transfer_request);
#else
    ...
    if (start)
    {
        ((struct UX_HCD_STRUCT *)hcd_ehci -> ux_hcd_ehci_hcd_owner) -> ux_hcd_thread_signal ++;
