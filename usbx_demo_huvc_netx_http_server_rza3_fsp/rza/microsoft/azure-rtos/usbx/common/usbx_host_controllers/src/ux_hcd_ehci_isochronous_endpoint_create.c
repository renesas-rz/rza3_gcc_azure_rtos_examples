...
UINT                            status;
    ...
    else
    {
        ...
        */
        ...
        for (i = 0; i < ed -> ux_ehci_hsiso_ed_nb_tds; i ++)
        {
            ...
            /* Link to ED.  */
            ((UX_EHCI_HSISO_TD *)itd.itd_ptr) -> ux_ehci_hsiso_td_ed = ed;

            /* Save max transfer size.  */
            ((UX_EHCI_HSISO_TD *)itd.itd_ptr) -> ux_ehci_hsiso_td_max_trans_size = (USHORT)max_trans_size;
        ...
        {
            for (i = 0; i < ed -> ux_ehci_hsiso_ed_nb_tds; i ++)
                ((UX_EHCI_HSISO_TD *)ed -> ux_ehci_hsiso_ed_fr_td[i]) -> ux_ehci_hsiso_td_status = UX_UNUSED;
...
#endif
    {
        ...
        for (i = 0; i < ed -> ux_ehci_hsiso_ed_nb_tds; i ++)
        {
            ...
            if (i < ed -> ux_ehci_hsiso_ed_nb_tds - 1u)
            {
                ...
                ((UX_EHCI_HSISO_TD *)itd.itd_ptr) -> ux_ehci_hsiso_td_next_lp = lp;
            }

            /* Build previous pointer, if not first one.  */
            if (i > 0)
            {
                ((UX_EHCI_HSISO_TD *)itd.itd_ptr) -> ux_ehci_hsiso_td_previous_lp.itd_ptr =
                                        ed -> ux_ehci_hsiso_ed_fr_td[i - 1];
            }

            /* Save Device Address and Endpt @ BP0.  */
            bp.value = device_address | endpt;
            ((UX_EHCI_HSISO_TD *)itd.itd_ptr) -> ux_ehci_hsiso_td_bp[0] = bp.void_ptr;

            /* Save I/O and max packet size @ BP1.  */
            bp.value = io | max_packet_size;
            ((UX_EHCI_HSISO_TD *)itd.itd_ptr) -> ux_ehci_hsiso_td_bp[1] = bp.void_ptr;

            /* Save Mult @ BP2.  */
            bp.value = mult;
            ((UX_EHCI_HSISO_TD *)itd.itd_ptr) -> ux_ehci_hsiso_td_bp[2] = bp.void_ptr;
        }
    ...
    if (device -> ux_device_speed != UX_HIGH_SPEED_DEVICE)
    {
        ...
        /* Save anchor pointer.  */
        ((struct UX_EHCI_FSISO_TD_STRUCT *)itd.sitd_ptr) -> ux_ehci_fsiso_td_anchor = ed_anchor;
        ...
        if (io == 0)
        {
            ...
            /* Save settings.  */
            ((UX_EHCI_FSISO_TD *)itd.sitd_ptr) -> ux_ehci_fsiso_td_cap1 = mask;
            ((UX_EHCI_FSISO_TD *)itd.sitd_ptr) -> ux_ehci_fsiso_td_back_pointer = lp.void_ptr;
        ...
        else
        {
            ...
            /* Single start split.  */
            ((UX_EHCI_FSISO_TD *)itd.sitd_ptr) -> ux_ehci_fsiso_td_cap1 = UX_EHCI_SMASK_0 << microframe_i;
...
#endif
    {
        ...
        for (i = microframe_i; i < 8; i += interval)
        {
            ...
            if (i & 1u)
                ((UX_EHCI_HSISO_TD *)itd.itd_ptr) -> ux_ehci_hsiso_td_control[i] = UX_EHCI_HSISO_IOC |
    ...
    ((UX_EHCI_HSISO_TD *)ed -> ux_ehci_hsiso_ed_fr_td[0]) -> ux_ehci_hsiso_td_previous_lp.ed_ptr = ed_anchor;
    ...
    if (device -> ux_device_speed != UX_HIGH_SPEED_DEVICE)
    {
        ((UX_EHCI_FSISO_TD *)itd.sitd_ptr) -> ux_ehci_fsiso_td_next_scan_td = hcd_ehci -> ux_hcd_ehci_fsiso_scan_list;
...
#endif
    {
        ...
        ((UX_EHCI_HSISO_TD *)itd.itd_ptr) -> ux_ehci_hsiso_td_next_scan_td =
                                hcd_ehci -> ux_hcd_ehci_hsiso_scan_list;
        hcd_ehci -> ux_hcd_ehci_hsiso_scan_list = itd.itd_ptr;
        if (((UX_EHCI_HSISO_TD *)itd.itd_ptr) -> ux_ehci_hsiso_td_next_scan_td)
            ((UX_EHCI_HSISO_TD *)((UX_EHCI_HSISO_TD *)itd.itd_ptr) -> ux_ehci_hsiso_td_next_scan_td) -> ux_ehci_hsiso_td_previous_scan_td =
                                itd.itd_ptr;
    }
