        ...
        if (_ux_system_host -> ux_system_host_hcd_array[hcd_index].ux_hcd_controller_type == UX_OHCI_CONTROLLER)
        {
            ...
            if (hcd -> ux_hcd_status == UX_HCD_STATUS_OPERATIONAL)
            {
                ...
                if (ohci_register & OHCI_HC_INT_WDH)
                {
                    ...
                    hcd_ohci -> ux_hcd_ohci_done_head =  ((UX_HCD_OHCI_HCCA *)hcd_ohci -> ux_hcd_ohci_hcca) -> ux_hcd_ohci_hcca_done_head;
                    ((UX_HCD_OHCI_HCCA *)hcd_ohci -> ux_hcd_ohci_hcca) -> ux_hcd_ohci_hcca_done_head =  UX_NULL;                    
