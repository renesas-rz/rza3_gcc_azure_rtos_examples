...
UX_EHCI_PERIODIC_LINK_POINTER     next_physical_lp;
    ...
    if (prev_next && next_prev)
    {
        ...
        ((UX_EHCI_HSISO_TD *)link_virtual_lp.itd_ptr) -> ux_ehci_hsiso_td_next_lp = next_physical_lp;
    }
    else
    {
    ...
    ((UX_EHCI_HSISO_TD *)prev_virtual_lp.itd_ptr) -> ux_ehci_hsiso_td_next_lp = link_physical_lp;
    ...
    if ((next_physical_lp.value & (UX_EHCI_LINK_ADDRESS_MASK | UX_EHCI_T)) != UX_EHCI_T)
    {
        ...
        case UX_EHCI_TYP_ITD:
            ((UX_EHCI_HSISO_TD *)virtual_lp.itd_ptr) -> ux_ehci_hsiso_td_previous_lp = link_virtual_lp;
            break;
#if defined(UX_HCD_EHCI_SPLIT_TRANSFER_ENABLE)
        case UX_EHCI_TYP_SITD:
            ((UX_EHCI_FSISO_TD *)virtual_lp.sitd_ptr) -> ux_ehci_fsiso_td_previous_lp = link_virtual_lp;
            break;
#endif
        default: /* QH.  */
            if ((((UX_EHCI_ED *)virtual_lp.ed_ptr) -> ux_ehci_ed_status & UX_EHCI_QH_STATIC) == 0)
                ((UX_EHCI_ED *)virtual_lp.ed_ptr) -> ux_ehci_ed_previous_ed = link_virtual_lp.ed_ptr;
            break;
        }
    }
}
