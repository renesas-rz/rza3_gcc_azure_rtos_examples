...
void mouse_thread_entry(ULONG thread_input)
{
    ...
    mouse_event.gx_event_payload.gx_event_pointdata.gx_point_x = PRIMARY_X_RESOLUTION/2;
    mouse_event.gx_event_payload.gx_event_pointdata.gx_point_y = PRIMARY_Y_RESOLUTION/2;
        ...
        else
        {
            ...
            if(status==UX_SUCCESS)
            {
                ...
                if(status==UX_SUCCESS)
                {
                    ...
                    else if(PRIMARY_X_RESOLUTION < current_x)
                    {
                        current_x = PRIMARY_X_RESOLUTION;
                    }
                    //y座標も同様に設定
                    if(current_y < 0)
                    {
                    ...
                    else if(PRIMARY_Y_RESOLUTION < current_y)
                    {
                        current_y = PRIMARY_Y_RESOLUTION;
                    }
